  ...
  async analyzeRepository(
    ...
    for (const fp of filePaths) {
      ...
      if (filesProcessed % 50 === 0) {
        onProgress?.({
          phase: 'indexing',
          progress: 20 + Math.round((filesProcessed / totalFiles) * 60),
          detail: `Indexed ${filesProcessed}/${totalFiles} files`,
          filesProcessed,
          totalFiles
        })
      }
    ...
    // 7. Build result (parsers will be added in Plan 13-02)
