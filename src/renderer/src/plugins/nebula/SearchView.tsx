...
import { Search, MessageCircleQuestion, FileText, Loader2, X } from 'lucide-react'
        ...
        <div className="mb-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-lg border border-border bg-surface-elevated px-3 py-2 pl-9 text-sm text-text-primary placeholder:text-text-secondary/50 focus:border-accent/50 focus:outline-none"
            />
            {isSearching && (
              <Loader2
                size={14}
        ...
        <div className="flex-1 space-y-2 overflow-y-auto">
          {searchQuery.trim() && searchResults.length === 0 && !isSearching ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 text-text-secondary">
              ...
              <p className="text-sm">Search your notes by keyword or phrase</p>
            </div>
          ) : (
            searchResults.map((result) => (
              <button
                ...
                className="w-full cursor-pointer rounded-lg border border-border/50 bg-surface-elevated p-3 text-left transition-colors hover:bg-surface-hover"
              >
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="mb-3 flex gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-border bg-surface-elevated px-3 py-2 text-sm text-text-primary placeholder:text-text-secondary/50 focus:border-accent/50 focus:outline-none disabled:opacity-50"
          />
          {isStreaming ? (
            <button
              type="button"
              onClick={cancelQa}
              className="flex items-center gap-1.5 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs font-medium text-red-400 transition-colors hover:bg-red-500/20"
            >
              <X size={13} />
              Cancel
            </button>
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg bg-accent/15 px-3 py-2 text-xs font-medium text-accent transition-colors hover:bg-accent/25 disabled:opacity-40 disabled:hover:bg-accent/15"
            >
        ...
        {/* Response area */}
        <div className="flex-1 overflow-y-auto rounded-lg border border-border/50 bg-surface p-3">
          {isStreaming && !qaAnswer ? (
            <div className="flex items-center gap-2 text-text-secondary">
              <Loader2 size={14} className="animate-spin text-accent" />
              <span className="text-sm">Searching notes and generating answer...</span>
            </div>
          ) : qaAnswer ? (
            <div>
              {isStreaming && (
            ...
            <div className="flex flex-col items-center justify-center py-6 text-text-secondary">
              <MessageCircleQuestion size={24} className="mb-2 opacity-40" />
              <p className="text-center text-sm">
                Ask anything about your notes. AI will search your knowledge base and provide an
                answer.
              </p>
            </div>
          )}
