  ...
  return (
    <TabsLayoutContext.Provider value={layoutId}>
      <TabsPrimitive.List
        data-slot="tabs-list"
        className={cn(
          "inline-flex h-10 items-center justify-center rounded-xl bg-white/5 border border-white/6 p-1 text-muted-foreground backdrop-blur-sm",
          className
        )}
        {...props}
      >
    ...
    >
      ...
      {isActive && (
        <motion.span
          ...
          className="absolute inset-0 rounded-lg bg-white/8 shadow-sm"
          style={{ zIndex: 0 }}
