...
const api = {
  ...
  app: {
    ...
    }): Promise<{ filePath: string | null }> => ipcRenderer.invoke('app:exportPdf', data),
    saveTextFile: (
      content: string,
  ...
  bitbucket: {
    ...
    postTopLevelComment: (
      ...
      ipcRenderer.invoke('bitbucket:postTopLevelComment', workspace, repoSlug, prId, comment),
  ...
  db: {
    ...
    getConnections: (): Promise<unknown[]> => ipcRenderer.invoke('db:getConnections'),
    ...
    buildSchemaContext: (
      ...
      schema: string,
      tables?: string[]
    ): Promise<string> => ipcRenderer.invoke('db:buildSchemaContext', connectionId, schema, tables),
  ...
  launchpad: {
    ...
    }): Promise<{ saved: boolean }> => ipcRenderer.invoke('launchpad:saveCredentials', credentials),
  ...
  textcraft: {
    exportPdf: (data: {
      ...
      title?: string
      mermaidImages?: Record<number, string>
    }): Promise<{ filePath: string | null }> => ipcRenderer.invoke('textcraft:exportPdf', data)
  },
  cortex: {
    ...
    probeRtk: (): Promise<boolean> => ipcRenderer.invoke('cortex:probeRtk'),
    // AI Enrichment
    buildDigest: (
      repoUrl: string,
      branch: string
  ...
  nebula: {
    ...
    loadNote: (id: string): Promise<unknown> => ipcRenderer.invoke('nebula:loadNote', id),
    listNotes: (): Promise<unknown[]> => ipcRenderer.invoke('nebula:listNotes'),
    deleteNote: (id: string): Promise<void> => ipcRenderer.invoke('nebula:deleteNote', id),
    searchNotes: (query: string): Promise<unknown[]> =>
      ipcRenderer.invoke('nebula:searchNotes', query),
    getGraph: (): Promise<unknown> => ipcRenderer.invoke('nebula:getGraph'),
